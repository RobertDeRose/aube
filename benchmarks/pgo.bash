...
find_llvm_prefix() {
	...
	if [ -z "$prefix" ]; then
		prefix=$(find /usr/lib -maxdepth 1 -type d -name 'llvm-[0-9]*' 2>/dev/null | sort -V -r | head -1)
	fi
	if [ -z "$prefix" ]; then
...
if [ -n "$PGO_BOLT" ]; then
	# Keep BOLT and merge-fdata on the same LLVM installation selected
	# above. Using the versioned prefix also preserves BOLT's runtime
	# lookup for /usr/lib/llvm-NN/lib/libbolt_rt_instr.a.
